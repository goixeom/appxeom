...
public class MapAnimator {
    ...
    static final int GREEN = Color.parseColor("#2fb35f");


    private MapAnimator(Context context) {
        this.context = context;
    }

    public static MapAnimator getInstance(Context context) {
        if (mapAnimator == null) mapAnimator = new MapAnimator(context);
        return mapAnimator;
    }


    public void animateRoute(GoogleMap googleMap, List<LatLng> bangaloreRoute, Polyline lines) {
        ...
        PolylineOptions optionsBackground = new PolylineOptions().add(bangaloreRoute.get(0)).color(GREY).width((int) context.getResources().getDimension(R.dimen.poliline_width));
        backgroundPolyline = googleMap.addPolyline(optionsBackground);

        optionsForeground = new PolylineOptions().add(bangaloreRoute.get(0)).color(Color.BLACK).width((int) context.getResources().getDimension(R.dimen.poliline_width));
        foregroundPolyline = googleMap.addPolyline(optionsForeground);

//        if (foregroundPolyline != null && backgroundPolyline != null && lines != null) {
//            lines.remove();
//        }
        final ValueAnimator percentageCompletion = ValueAnimator.ofInt(0, 100);
        percentageCompletion.setDuration(3000);     // Chạy trong bao nhiêu s
        ...
        foregroundPolyline.setPoints(bangaloreRoute);
...
//        foregroundRouteAnimator.start();
        ...
        secondLoopRunAnimSet.playSequentially(colorAnimation,
                percentageCompletion);
        secondLoopRunAnimSet.setStartDelay(600);
